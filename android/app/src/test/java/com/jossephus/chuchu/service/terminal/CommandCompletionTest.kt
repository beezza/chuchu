...
class CommandCompletionTest {
    ...
    @Test
    fun crlfInputCountsAsOneCommand() {package com.jossephus.chuchu.service.terminal
...
class CommandCompletionTest {
    ...
    @Test
    fun parserRecognizesAgentCompletionEvents() {
        assertEquals(
            ChuchuControlEvent.AgentDone(0),
            ChuchuControlEventParser.parse("chuchu-agent-done;status=0"),
        )
        assertEquals(
            ChuchuControlEvent.AgentDone(0),
            ChuchuControlEventParser.parse("agent-turn-complete"),
        )
        assertEquals(
            ChuchuControlEvent.AgentDone(0),
            ChuchuControlEventParser.parse("Agent turn complete"),
        ...
        assertEquals(
            ChuchuControlEvent.AgentDone(0),
            ChuchuControlEventParser.parse("Pi: Ready for input"),
        )
        assertEquals(
            ChuchuControlEvent.AgentDone(0),
            ChuchuControlEventParser.parse("Ready for input"),
        )
        assertNull(ChuchuControlEventParser.parse("Ready for input: not an agent"))
    }

    @Test
    fun agentCompletionUsesOldestMarkerAndDropsStalePromptMarkers() {
        var now = 0L
        val tracker = CommandCompletionTracker(minimumDurationMs = 0L, nowMs = { now })
        tracker.recordInput("\r".toByteArray())
        ...
        now = 5_000L
        assertEquals(
            CompletedCommand(exitCode = 0, durationMs = 5_000L),
            tracker.agentCompleted(exitCode = 0),
        )
        ...
        now = 6_000L
        assertEquals(
            CompletedCommand(exitCode = 0, durationMs = 900L),
            tracker.agentCompleted(exitCode = 0),
        )
        assertNull(tracker.agentCompleted(exitCode = 0))
    }
