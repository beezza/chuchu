...
const COMMAND_SHELL_EVENT_PREFIX = "chuchu-shell;";
const COMMAND_DONE_EVENT_PREFIX = "chuchu-command-done;";
const AGENT_DONE_EVENT_PREFIX = "chuchu-agent-done;";
const CODEX_AGENT_EVENT_PREFIX = "agent-turn-complete";
const CODEX_AGENT_EVENT_TEXT_PREFIX = "Agent turn complete";
const CODEX_AGENT_EVENT_TEXT_LOWER_PREFIX = "agent turn complete";
const CODEX_AGENT_EVENT_CODEX_PREFIX = "Codex: Agent turn complete";
const PI_AGENT_EVENT_PREFIX = "Pi: Ready for input";
const PI_AGENT_EVENT_SHORT_PREFIX = "Ready for input";
const PI_AGENT_EVENT_COMPLETE_PREFIX = "Pi turn complete";
// Kitty Unicode graphics placeholder (U+10EEEE). These cells only mark where an
// image is composited; we blank them so the raw glyph never shows through a gap
// the image doesn't fully cover.
const KITTY_PLACEHOLDER_CP: u32 = 0x10EEEE;
...
const ChuchuStreamHandler = struct {
    ...
    fn commandNotification(self: *ChuchuStreamHandler, body: []const u8) bool {
        if (!isCommandNotification(body)) return false;
        if (body.len == 0 or body.len > MAX_COMMAND_EVENT_BYTES) return true;
        const owner = self.owner;
        const extra = body.len + 1;
        if (extra > MAX_COMMAND_EVENT_BUFFER_BYTES or
...
fn isCommandNotification(body: []const u8) bool {
    return std.mem.startsWith(u8, body, COMMAND_SHELL_EVENT_PREFIX) or
        std.mem.startsWith(u8, body, COMMAND_DONE_EVENT_PREFIX) or
        std.mem.startsWith(u8, body, AGENT_DONE_EVENT_PREFIX) or
        std.mem.startsWith(u8, body, CODEX_AGENT_EVENT_PREFIX) or
        std.mem.startsWith(u8, body, CODEX_AGENT_EVENT_TEXT_PREFIX) or
        std.mem.startsWith(u8, body, CODEX_AGENT_EVENT_TEXT_LOWER_PREFIX) or
        std.mem.startsWith(u8, body, CODEX_AGENT_EVENT_CODEX_PREFIX) or
        std.mem.startsWith(u8, body, PI_AGENT_EVENT_PREFIX) or
        std.mem.startsWith(u8, body, PI_AGENT_EVENT_SHORT_PREFIX) or
        std.mem.startsWith(u8, body, PI_AGENT_EVENT_COMPLETE_PREFIX);
}
...
ABCexport fn shm_open(name: [*:0]const u8, oflag: c_int, mode: c_uint) callconv(.c) c_int {const std = @import("std");
const ghostty = @import("ghostty-vt");
const zignal_png = @import("zignal_png.zig");
